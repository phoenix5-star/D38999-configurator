...
public class DisassemblerGender {
        ...
        while (i < il.Length) {
            ...
            switch (op.OperandType) {
                ...
                case OperandType.InlineTok:
                    ...
                    i += 4;
                    try { MemberInfo mem = module.ResolveMember(token); extra = mem.DeclaringType.Name + "." + mem.Name; } catch { extra = "0x" + token.ToString("X"); }
                    break;
                case OperandType.InlineString:
                    int strToken = BitConverter.ToInt32(il, i);
                    i += 4;
                    try { extra = "\"" + module.ResolveString(strToken) + "\""; } catch { extra = "str 0x" + strToken.ToString("X"); }
                    break;
...
[DisassemblerGender]::PrintMethod($type.GetMethod("get_Gender"))
[DisassemblerGender]::PrintMethod($type.GetMethod("set_Gender"))
