...
public class DisassemblerPropCtor {
    public static void PrintMethod(ConstructorInfo m) {
        if (m == null) return;
        Console.WriteLine("--- " + m.DeclaringType.Name + " .ctor (" + m.GetParameters().Length + " params) ---");
...
foreach ($m in $type.GetConstructors($flags)) {
    [DisassemblerPropCtor]::PrintMethod($m)
}
