...
public class DisassemblerSetValue {
    public static void PrintMethod(MethodInfo m, StreamWriter sw) {
        if (m == null) return;
        sw.WriteLine("--- " + m.DeclaringType.Name + "." + m.Name + " (" + m.GetParameters().Length + " params) ---");
        ...
        while (i < il.Length) {
            ...
            sw.WriteLine(string.Format("IL_{0:X4}: {1} {2}", offset, op.Name, extra));
        }
    }
}
"@
...
$type = [SkyCadKernel.SkyCadPropertyDefinition]
$sw = New-Object System.IO.StreamWriter("scratch\setvalue_disasm.txt")
foreach ($m in $type.GetMethods()) {
    if ($m.Name -eq "SetValue") {
        [DisassemblerSetValue]::PrintMethod($m, $sw)
    }
}
$sw.Close()
Write-Host "Wrote setvalue_disasm.txt"
